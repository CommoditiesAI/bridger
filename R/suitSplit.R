...
suitSplit <- function(missingCards = 5, cards_W = 13, cards_E = 13) {
  ...
  graph <- ggplot(outTable) +
    ...
    geom_label(aes(x = xaxis, y = Probability, label = scales::percent(Probability, accuracy = 0.1),
                   vjust = ifelse(Probability > 0.1, 1.6, -0.4)), fill = "white", fontface = "bold", label.size = NA) +
